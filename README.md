...
This project is licensed under Apache-2.0 and includes a LICENSE_APPENDIX.md that clarifies intended use and boundaries. The protocol and its modules (including ILRM) are non-adjudicative coordination tools, not courts, judges, or legally binding arbitration systems. Any proposals, reconciliations, or license adjustments generated by the system—whether algorithmic or LLM-assisted—are non-authoritative and non-binding, and require explicit participant acceptance. Economic mechanisms such as staking, escrow, or burning exist solely to align incentives and discourage prolonged conflict, not to impose legal penalties or determine fault. Certain commercial or hosted uses may require a separate license, and project names and identifiers may not be used in a way that implies official endorsement.
